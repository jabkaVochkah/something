...
USER_CONFIG_YAML_URL="https://raw.githubusercontent.com/jabkaVochkah/something/main/config.yaml"
# ---------------------------------
...
echo -e "\n--- Подготовка config.yaml из официального шаблона ---"
# Копируем официальный дефолтный конфиг, который является частью SillyTavern
cp default/config.yaml config.yaml

# Модифицируем config.yaml с помощью sed
# listen: false
...
# protocol: ipv6: false (убедимся, что IPv6 точно выключен для слушающего сервера)
sed -i '/protocol:/!b;n;s/    ipv6: true/    ipv6: false/' config.yaml # Ищет protocol:, затем следующую строку с ipv6: true и меняет на false

echo "config.yaml успешно подготовлен и настроен."

echo -e "\n--- Установка и запуск SillyTavern с PM2 ---"
# Убедимся, что pm2 установлен
sudo -E env PATH="$PATH" npm install -g pm2

# Удаляем любой существующий PM2-процесс SillyTavern, чтобы обеспечить чистый старт
...
# Сохраняем PM2-конфигурацию и настраиваем автозапуск
pm2 save
pm2 startup systemd
...
sleep 10
echo "Инициализация SillyTavern завершена."
